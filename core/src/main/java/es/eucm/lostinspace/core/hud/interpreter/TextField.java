 ...
 * pressed. Clipboard functionality is provided via the {@link com.badlogic.gdx.utils.Clipboard} interface. Currently there are
 * two standard implementations, one for the desktop and one for Android. The Android clipboard is a stub, as copy & pasting on
...
public class TextField extends Widget {
	...
	protected float keyRepeatInitialTime = 0.4f;
	protected float keyRepeatTime = 0.1f;
	...
	protected void initialize () {
		addListener(inputListener = new TextFieldBasisListener());
	}
	...
	/** Copies the contents of this TextField to the {@link com.badlogic.gdx.utils.Clipboard} implementation set on this TextField. */
	public void copy () {
		if (hasSelection) {
			int minIndex = Math.min(cursor, selectionStart);
			int maxIndex = Math.max(cursor, selectionStart);
	...
	/** Copies the selected contents of this TextField to the {@link com.badlogic.gdx.utils.Clipboard} implementation set on this
	 * TextField, then removes it. */
	public void cut () {
		if (hasSelection) {
			copy();
			delete();
	...
	/** Pastes the content of the {@link com.badlogic.gdx.utils.Clipboard} implementation set on this Textfield to this TextField. */
	...
	public void next (boolean up) {
		...
		getParent().localToStageCoordinates(tmp.set(getX(), getY()));
		TextField textField = findNextTextField(stage.getActors(), null, tmp2, tmp, up);
		if (textField == null) { // Try to wrap around.
			if (up)
				tmp.set(Float.MIN_VALUE, Float.MIN_VALUE);
			else
				tmp.set(Float.MAX_VALUE, Float.MAX_VALUE);
			textField = findNextTextField(getStage().getActors(), null, tmp2, tmp, up);
		}
		if (textField != null)
	...
	public class KeyRepeatTask extends Task {
		public int keycode;
	 ...
	 * {@link com.badlogic.gdx.Input#setOnscreenKeyboardVisible(boolean)} as appropriate. Might overlap your actual rendering, so
	 * use with care!
	 * @author mzechner */
	...
	public class TextFieldBasisListener extends ClickListener {
