...
public class Parser {
	...
	public boolean checkAttributes(XMLNode n, boolean strict) {
		...
		return checkAttributes(n, requiredAtt == null ? EMPTY : requiredAtt,
				impliedAtt == null ? EMPTY : impliedAtt, strict);
	}
	 ...
	 * @return if
	 *         Checks if the attribute has an admitted value
	 */
		...
		if (att != null) {
			for (Map.Entry<String, String> e : att.entrySet()) {
				if (contains(e.getKey(), requiredAtt) >= 0) {
					...
					error(e.getKey()
							+ PhaseScreen.i18n(" is not a valid attribute name for node ")
							+ n.getNodeName());
					ok = false;
				}
			}
			...
			if (required < requiredAtt.length) {
				ok = false;
				error(n.getNodeName()
						+ PhaseScreen.i18n(" is missing some required attributes."));
			}
		}
	...
	private boolean isRequiredAttribute(XMLNode node, String attName) {
		if (this.requiredAtts.get(node.getNodeName()) != null &&
				contains(attName, this.requiredAtts.get(node.getNodeName())) >= 0) {
	...
	/**
	 * Returns an integer for an attribute, between a max and a min. If some exception happens,
	 * returns null. If the value is null and it's a required attribute, null is returned, min otherwise
	 *
	 * @param node    the node
	 * @param attName the attribute name
	...
	/**
	 * Returns an integer for an attribute, between a max and a min. If some exception happens or the value is null,
	 * returns null
	 *
	 ...
	 * @return the attribute value
	 */
	public Float getFloat(XMLNode node, String attName, float min,
						  float max) {
		String value = getAttribute(node, attName);
		if (value == null) {
			return isRequiredAttribute(node, attName) ? null : min;
	...
	/**
	 * Returns a boolean for an attribute. If some exception happens or the value is null, null is returned. If the
	 * value is null, false is returned if it's not a required attrbitue, null otherwise
	 *
